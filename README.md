...
Python (`uv`) と Node.js (`atcoder-cli`) を組み合わせた構成になっています。

## 1. 環境構築

### Python 環境 (uv)
このプロジェクトでは Python のパッケージ管理に `uv` を使用しています。
...
### Node.js 環境 (atcoder-cli)
コマンドラインツール `acc` をインストールします。
...
独自ラッパースクリプト `ac` を使用して、取得・テスト・提出をシームレスに行えます。
